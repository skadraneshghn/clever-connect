...
import { 
	...
	FiRotateCcw, FiVolume2, FiVolumeX, FiTv, FiMinimize2, FiCloud
} from 'react-icons/fi';
...
export const FilesPage: React.FC = () => {
	...
	const handleCopy = () => {
		...
		showGlobalAlert(`Copied ${selectedItems.length} items to clipboard.`, { title: 'Copied to Clipboard', variant: 'success' });
	};
	...
	const handleCut = () => {
		...
		showGlobalAlert(`Cut ${selectedItems.length} items to clipboard.`, { title: 'Cut to Clipboard', variant: 'success' });
	};
	...
	const handleRename = async (file: FileItem) => {
		const newName = await showGlobalPrompt(`Enter new name for "${file.name}":`, file.name, { title: 'Rename File' });
	...
	const handleExtract = async (file: FileItem) => {
		if (!(await showGlobalConfirm(`Do you want to extract "${file.name}" in the current directory?`, { title: 'Extract Archive' }))) return;
	...
	const handleAddToCompressQueue = (file: FileItem) => {
		...
		setCompressQueue(prev => {
			if (prev.some(item => item.path === fullPath)) {
				showGlobalAlert(`"${file.name}" is already in the compress queue.`, { title: 'Queue Info', variant: 'info' });
				return prev;
			}
	...
	const handleDeleteSelected = async () => {
		if (selectedItems.length === 0) return;
		if (!(await showGlobalConfirm(`Are you absolutely sure you want to permanently delete these ${selectedItems.length} items?`, { title: 'Delete Selected Items', variant: 'warning' }))) return;
	...
	const handleSaveContent = async () => {
		...
		try {
			...
			if (res.ok) {
				showGlobalAlert('File saved successfully!', { title: 'File Saved', variant: 'success' });
			}
		} catch (err) {
			console.error(err);
		} finally {
